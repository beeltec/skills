...
4. **Ship** — invoke `$implement-with-subagents` with the resulting `EPIC-NNN` or `WORK-NNN` set and the transcript path. It assigns every work item to a fresh implementation subagent, running independent conflict-disjoint items concurrently and all others serially. Never `$implement` directly or implement in this context.
