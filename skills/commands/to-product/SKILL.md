...
End the report with `Next step:` — one copy-pasteable command: parked items → the exact command that resumes the highest-ranked one; unshipped map outcomes → `/to-product` with the same PRD to resume; out-of-PRD records filed → `/to-backlog` naming them; otherwise omit it. Recommend only — never invoke it; make it the last line, or a numbered list in run order if several apply.
