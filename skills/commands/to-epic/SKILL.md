...
Pause for user input only at the evidence decision (step 2), on a `$guidance` rule-replacement pause, and on hard blockers; otherwise run to completion and report. The rule-replacement pause is never covered by any standing approval — never suppress it to preserve the single pause.
