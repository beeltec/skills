...
Before moving or deleting a concept, repair all wiki references in the same transaction. An active backlog reference blocks the operation until `$develop plan ...` applies a separately approved reference update. Archived backlog records may retain missing historical `wiki_refs`; validation reports those references as warnings.
...
- `engineering/guidance-template.md` is the shared non-record structure for both. Create and refresh those pages with `$develop guidance ...`; it is a non-record example and states no accepted guidance itself.
