...
`docs/backlog` is the tracked system of record for desired project deltas and their execution state. `docs/wiki` is the canonical record of accepted current state on the primary branch. A completed backlog item does not redefine current state by itself: after primary-branch acceptance, publish the approved `$develop knowledge ...` reconciliation before completing and archiving the backlog records.
