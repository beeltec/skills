...
5. Where applicable, also research current best practices and recommended coding guidelines for each affected technology — official style guides, idiomatic usage and configuration patterns, security recommendations, and maintainer-recommended project conventions — using the same source order and version-matching rules. Skip only when inspected evidence shows the proposed delta touches no code or configuration the guidelines would govern.
