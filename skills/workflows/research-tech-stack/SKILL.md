...
- During implementation reconciliation, promote only conclusions that became durable accepted current guidance to the owning wiki concept. Use `$to-wiki` under its approval rules, summarize current guidance rather than copying the proposal record, and retain the backlog research as history.
